...
import { changeOwnerPassword, fetchOwnerProfile, updateOwnerProfileDetails } from "../../features/ownerSlice/ownerSlice";
...
const OwnerProfile = () => {
  ...
  const { profile, loading } = useSelector((state) => state.owner);
    ...
    useEffect(() => {
        if (profile) {
          setFormData({
            ...
            email: profile.email || "",
            correspondedMe: profile.correspondedMe || "",
  ...
  const handleSave = (e) => {
    ...
    formDataToSend.append("correspondedMe", formData.correspondedMe);

    if (profileImage) {
      formDataToSend.append("avatar", profileImage);
    }

    dispatch(updateOwnerProfileDetails(formDataToSend))
      .unwrap()
      .then(() => {
  ...
  const handlePassChange = () => {
    ...
    dispatch(changeOwnerPassword({ currentPassword, newPassword }))
      .unwrap()
      .then(() => {
  ...
  return (
    <div className="bg-[#F5F5F5] ml-56 p-6 min-h-screen  mx-auto mt-16 font-[Inter]">
        ...
        <h2 className="text-3xl font-semibold font-[Poppins] text-[#363B64] mb-4">Edit Profile</h2>
          ...
          {activeTab === "profile" ? (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                ...
                {/* Country */}
                <div>
                  <label className="text-sm">Corresponded Me</label>
                  <input
                    type="text"
                    name="correspondedMe"
                    value={formData.correspondedMe}
                    onChange={handleProfileChange}
                    placeholder="Enter What Corresponded You"
                    className="w-full border border-gray-200 rounded-xl text-[#718EBF] focus:ring-1 focus:ring-[#DFEAF2] outline-none px-3 py-2.5 mt-1 text-sm"
                  />
                </div>
